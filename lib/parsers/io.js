...
module.exports = (getParsers) => {
  ...
  const doFuncParser = input => maybe(
    ...
    (val, rest) => {
      ...
      funcBody = notUndefined(funcBody.expression) && funcBody.expression.type === AST.ARROW_FUNCTION_EXPRESSION ? funcBody.expression.body : funcBody
      funcBody.sType = 'IO'
  ...
  const isIOorFuncCall = maybeIOFunc => {
    const isFuncCall = notUndefined(maybeIOFunc.type, maybeIOFunc.expression) && maybeIOFunc.expression.type === AST.CALL_EXPRESSION
    const isIOCall = notUndefined(maybeIOFunc.type) && maybeIOFunc.type === AST.IDENTIFIER
  ...
  const bindStmt = (maybeBind, parentObj, bindBody, mapBody) => {
    ...
    const isNewIO = notNull(ioFunc) && ioFunc.name === 'IO' && args[0].type === AST.CALL_EXPRESSION
  ...
  const getCbBody = (bindBody, parentObj) => {
    if (bindBody.length === 0) {
      if (parentObj.type === AST.IDENTIFIER) return estemplate.fnCall(parentObj, [])
      return parentObj
    }
  ...
  const makeFinalStmt = (input, rest, parentObj, bindBody, mapBody) => {
    ...
    if (isUndefined(parentObj.returnVals)) {
      ...
      if (!isEmptyArr(mapBody.stmts) && !isEmptyArr(cbBody)) {
        const cbExpr = callBack.type === AST.ARROW_FUNCTION_EXPRESSION ? callBack.callee : callBack.expression
  ...
  const ioParser = input => maybe(
    ...
    (initIO, rest) => {
      ...
      const isId = notUndefined(maybeDo.type) && maybeDo.type === AST.IDENTIFIER
      const idNotMain = isId && doID.name !== 'main'
      if (idNotMain) return null
      const isFunc = notUndefined(maybeDo.type) && maybeDo.type === AST.CALL_EXPRESSION
