...
const evalFunction = (argsObj, infile, outfile) => {
  ...
  if (parseResult !== null) {
    ...
    importCore = fs.readFileSync(importPath + 'core.js', 'utf8').toString() +
...
module.exports = argsObj => {
  ...
  if (watchFolder) {
    ...
    .on('addDir', addedDir => createFolder(outfile, addedDir, watchBase))
    .on('add', addedFile => fileWatcher(argsObj, outfile, addedFile, watchBase))
    .on('change', changedFile => fileWatcher(argsObj, outfile, changedFile, watchBase))
    .on('unlinkDir', unlinkedDir => removeFolder(outfile, unlinkedDir, watchBase))
    .on('unlink', unlinkedFile => removeFile(outfile, unlinkedFile, watchBase))
  }
}
