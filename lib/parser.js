...
/*
  All statements with `.expression` are used to unwrap an expression from within an `ExpressionStatement`
  All IO is converted into a syntax tree that can be handled by `node-core`/`browser-core`
*/
...
const {
  ...
  numberParser, nonReservedIdParser, identifierParser, domMethodParser, nullParser, stringParser, booleanParser, regexParser,
...
const defineStmtParser = input => maybe(
  parser.all(definePropParser, spaceParser,
             ...
             parser.any(stringParser, nonReservedIdParser, valueParser), spaceParser, valueParser)(input),
  (val, rest) => {
...
const makeMap = (parentObj, mapBody) => {
  ...
  let returnVal = makeReturnArray(nextParams.concat(mapBody.propagate))
  let cbBody = makeBlock(mapBody, returnVal)
...
const programParser = (input, ast = estemplate.ast()) => {
  ...
  if (isNull(result)) {
    let errObj = { ...parser.unParsed, regex: [...parser.unParsed.regex] }
